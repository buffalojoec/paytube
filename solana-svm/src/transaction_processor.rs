...
impl TransactionBatchProcessor {
    ...
    pub fn load_and_execute_sanitized_transactions<L: Loader>(
        ...
        let mut program_account_keys =
            filter_executable_program_accounts(sanitized_txs, loader, PROGRAM_OWNERS, config);

        if let Some(builtin_program_ids) = config.builtin_program_ids {
            builtin_program_ids.iter().for_each(|id| {
        ...
        let mut loaded_transactions = load_transactions(
            sanitized_txs,
            loader,
        ...
        let execution_results: Vec<TransactionExecutionResult> = loaded_transactions
            ...
            .map(|(load_result, tx)| match load_result {
                ...
                Ok(loaded_transaction) => match config.compute_budget {
                    Some(compute_budget) => execute_loaded_transaction(
                        tx,
                        loader,
                        loaded_transaction,
                        config,
...
fn execute_loaded_transaction<L: Loader>(
    ...
    for program_indices in loaded_tx.program_indices.iter() {
        for index in program_indices.iter() {
            let (program_id, program_account) = transaction_accounts.get(*index as usize).unwrap();
            let account_owner = ProgramCacheEntryOwner::try_from(program_account.owner())
                .expect("Invalid program owner");
            if let Some(executable) = loader.load_program(program_id, config) {
    ...
    let sysvar_cache = loader.vend_sysvar_cache(config);
    ...
    let return_data =
        if config.recording_config.enable_return_data_recording && !return_data.data.is_empty() {
            Some(return_data)
        } else {
            None
        };
    ...
    sanitized_txs
        ...
        .flat_map(|tx| {
            tx.message()
                .account_keys()
                .iter()
                .filter(|key| loader.account_matches_owners(key, program_owners, config))
                .copied()
        })
        .collect()
}

fn load_transactions<L: Loader>(
    sanitized_txs: &[SanitizedTransaction],
    loader: &L,
    ...
    sanitized_txs
        ...
        .map(|tx| {
            ...
            loader.load_transaction_accounts(
                message,
