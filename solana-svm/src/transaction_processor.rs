...
impl TransactionBatchProcessor {
    ...
    pub fn load_and_execute_sanitized_transactions<L: Loader>(
        ...
        let mut program_account_keys =
            filter_executable_program_accounts(sanitized_txs, loader, PROGRAM_OWNERS);
...
fn execute_loaded_transaction<L: Loader>(
    ...
    for program_indices in loaded_tx.program_indices.iter() {
        for index in program_indices.iter() {
            ...
            if let Some(executable) = loader.load_program(program_id, config.feature_set) {
    ...
    let sysvar_cache = loader.vend_sysvar_cache();
    ...
    sanitized_txs
        ...
        .flat_map(|tx| {
            tx.message()
                .account_keys()
                .iter()
                .filter(|key| loader.account_matches_owners(key, program_owners))
                .copied()
        })
        .collect()
}
